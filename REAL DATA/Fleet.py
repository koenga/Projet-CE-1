...
class Monitoring_Fleet():
    ...
    def __init__(self, Map, buffer_size=None, state_shape=None, pretrained_folder=None, lstm=False, gru = True, num_stacked=3, alpha=1e-4):
        ...
        if self.lstm:
            ...
            self.policy_network = GRU_model(input_size=state_shape[1], hidden_size=self.state_shape[1], num_stacked_layers=num_stacked, alpha=self.alpha)
            self.target_network = GRU_model(input_size=state_shape[1], hidden_size=self.state_shape[1], num_stacked_layers=num_stacked, alpha=self.alpha)            
