...
class Monitoring_Fleet():
    ...
    def __init__(self, Map, buffer_size=None, state_shape=None, pretrained_folder=None, lstm=False, gru = True, num_stacked=3, alpha=1e-4):
        ...
        if self.lstm:
            ...
            self.policy_network = GRU_model(input_size=state_shape[1], hidden_size=13, num_stacked_layers=num_stacked, alpha=self.alpha)
            self.target_network = GRU_model(input_size=state_shape[1], hidden_size=13, num_stacked_layers=num_stacked, alpha=self.alpha)            
...
#----- RL functions -----
    ...
    def continue_training(self, ckpt_file_name, lr=1e-4, n_lstm=2, N_iter=3000, load_data='/Results_/06_19_2024_20_28_04', pretrained_folder="/Results/06_20_2024_01_31_10_pretraining/Saved_models/policy_network_29999.pt", plot_pretrained=True):
        ...
        if self.lstm:
            ...
            self.policy_network = GRU_model(input_size=self.state_shape[1], hidden_size=13, num_stacked_layers=n_lstm, alpha=lr)
            self.target_network = GRU_model(input_size=self.state_shape[1], hidden_size=13, num_stacked_layers=n_lstm, alpha=lr)            
